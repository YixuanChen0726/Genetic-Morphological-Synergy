...
HighData=ExtremeFateList{2,3};
LowData=ExtremeFateList{3,3};

viewangle=[7,26;-18,7;-30,19;0,21;158,-7];
for Num=1:5
    figure(Num)
    ISphHigh=HighData(Num,:);
    SampleNum=ISphHigh{1,1};CellName=ISphHigh{1,2};Frame=ISphHigh{1,3};
    ...
    print(gcf,['.\Sample',num2str(SampleNum),'_',CellName,'_Frame_',num2str(Frame,'%03d'),'_ISphHigh',num2str(Num)],'-dpng','-r600');
end

viewangle=[65,-13;-134,-6;-108,10;81,-12;-2,9];
for Num=1:5
    figure(Num+5)
    ISphLow=LowData(Num,:);
    SampleNum=ISphLow{1,1};CellName=ISphLow{1,2};Frame=ISphLow{1,3};
    CellIndex=NameDic{find(strcmp(NameDic(:,2),CellName)),1};
    load(['..\..\..\bin\CMap Data\WT_Sample',num2str(SampleNum),'_AllSpace.mat']);
    Space=AllSpace{1,Frame};
    ind=find(Space==CellIndex);[X,Y,Z]=ind2sub(size(Space),ind);
    ...
    set(gcf,'unit','centimeters','position',[10,5,8,6]);
    set(gca,'unit','centimeters','position',[2,1,5,3.5]);
    print(gcf,['.\Sample',num2str(SampleNum),'_',CellName,'_Frame_',num2str(Frame,'%03d'),'_ISphLow',num2str(Num)],'-dpng','-r600');
end